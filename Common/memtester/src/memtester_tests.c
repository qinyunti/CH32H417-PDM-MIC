...
int test_stuck_address(ulv *bufa, size_t count) {
    ...
    for (j = 0; j < STUCK_LOOPS; j++) {
        ...
        for (i = 0; i < count; i++, p1++) {
            ul tmp = *p1;
            ul check = (((j + i) % 2) == 0 ? (ul) p1 : ~((ul) p1));
            if (tmp != check) {
                memtester_printf("\r\nFAILURE: possible bad address line at physical address " FMT_TARGET" "FMT_TARGET"!=" FMT_TARGET ".\n",
                    (ul)bufa + (i * sizeof(ul)), check, tmp);
